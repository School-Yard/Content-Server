...
module.exports = function(options) {
  var _resource = options.adapters.mongo.resource('pages');

  var Page = function Page(attrs) {
  ...
  Page.prototype.validate = function(callback) {
    ...
    if(errors.length === 0) {
      ...
      if(this._changed.indexOf('slug') !== -1) {
        ...
        Page.find({ slug: this.get('slug') }, function(err, result) {
      ...
      return callback(new Error('Invalid Page data'));
    }
  };
   ...
   * All Pages
   *
   * Gets all pages in the database.
   *
  ...
  Page.get = function(id, callback) {
    _resource.get(id, function(err, page) {
      if(err) return callback(err);
      if(!page || Object.keys(page).length === 0) return callback(null, null);
   ...
   * Create a Page
   *
   * Inserts a page record into the Database
   *
