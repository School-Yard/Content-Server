...
Properties.index = function index(req, res) {
  var props,
      params = req.params;
...
Properties.show = function show(req, res) {
  ...
  async.waterfall([
    ...
    // Find Property
    function(item, callback) {
      Property.get(params.id, function(err, property) {
        if(err) return callback(err);
...
Properties.create = function create(req, res) {
  var data,
      params = req.params;
...
Properties.update = function update(req, res) {
  var data,
      params = req.params;
...
Properties.destroy = function destroy(req, res) {
  var params = req.params;
...
var createProperty = function(properties, item) {
  return function(prop, callback) {
    ...
    Property.create(prop, function(err, item_prop) {
      if(err) return callback(err);
...
var validateProperty = function(item_id, errors, properties) {
  return function(prop, callback) {
    // Find the property
    Property.getByKey(item_id, prop.key, function(err, result) {
      if(err) return callback(err);
