...
Buckets.show = function show(req, res) {
  if(!req.params.bucket_name) return res.json(404, { error: "Can't find that bucket" });

  Bucket.find({ name: req.params.bucket_name }, function(err, buckets) {
    if(err) return res.json(500, { error: err.message });
...
Buckets.update = function update(req, res) {
  if(!req.params.name) return res.json(500, { error: "Can't find that bucket" });
  if(!req.body) return res.json(400, { error: 'invalid json' });
...
function collectProperties(properties) {
  return function(item, callback) {
    ItemProps.find({ item_id: item._id }, function(props) {
