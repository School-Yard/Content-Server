...
module.exports = function(adapter) {
  ...
  ItemProperty.prototype.validate = function(callback) {
    ...
    if(errors.length === 0) {
      ...
      if(this._changed.indexOf('key') !== -1) {
        ...
        ItemProperty.getByKey(this.get('item_id'), this.get('key'), function(err, result) {
  ...
  /**
   * Get Single ItemProperty by item_id and key
   *
   * Returns a single `ItemProperty` object
   *
   * @param {String} id
